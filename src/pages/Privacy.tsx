...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    paragraphs: [
      'Сайтът използва строго необходими бисквитки, за да работи коректно, и — само с ваше съгласие — допълнителни бисквитки от вградени услуги (напр. календара за резервации и картата).',
      'При първо посещение можете да приемете или да откажете незадължителните бисквитки чрез банера в долната част на екрана. Можете да оттеглите съгласието си по всяко време, като изчистите бисквитките в браузъра си.',
    ],
  },
  {
      ...
      <div className="mx-auto" style={{ maxWidth: '760px' }}>
        ...
        >
          <header>
            ...
            />
            <p className="text-xs tracking-[0.06em]" style={{ color: 'rgba(242,237,226,0.5)' }}>
          ...
          {SECTIONS.map((s) => {
            ...
            return (
              <section className="mt-11" key={s.title}>
                <h2 className="font-serif-luxe" style={{ fontSize: '20px', lineHeight: 1.3, color: '#e7cfa0' }}>
                  {s.title}
                </h2>

                {s.paragraphs?.map((p, i) => (
                  <p key={i} className="mt-4" style={{ fontSize: '15px', lineHeight: 1.7, color: 'rgba(242,237,226,0.82)' }}>
                    {p}
                  </p>
                ))}
                  ...
                  <ul className="mt-4 flex flex-col gap-2.5">
                    {s.list.map((item, i) => (
                      <li
                        ...
                        style={{ fontSize: '15px', lineHeight: 1.6, color: 'rgba(242,237,226,0.82)' }}
                      >
                        <span aria-hidden="true" className="mt-[9px] flex-none" style={{ width: '5px', height: '5px', borderRadius: '50%', background: 'var(--accent)' }} />
                        <span>
                          {item.term && <strong style={{ color: '#f2ede2', fontWeight: 500 }}>{item.term}: </strong>}
                          {item.text}
                        </span>
                      </li>
                    ))}
